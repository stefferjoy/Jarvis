...
class JarvisOrchestrator:
    ...
    def _handle_shortcut(self, message: str) -> str:
        ...
        shortcut_name = self._extract_shortcut_name(normalized)
    ...
    def _is_shortcut_intent(normalized: str) -> bool:
        return JarvisOrchestrator._extract_shortcut_name(normalized) is not None

    @staticmethod
    def _extract_shortcut_name(normalized: str) -> str | None:
        patterns = [
            r"^(?:run|start|trigger)\s+(?:shortcut|automation)\s+(.+)$",
            r"^(?:run|start|trigger)\s+(.+?)\s+(?:shortcut|automation)$",
        ]
