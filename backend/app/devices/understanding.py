...
class DeviceUnderstanding:
    ...
    def _score_device(self, device: dict[str, Any], normalized_utterance: str) -> int:
        ...
        if canonical_name and canonical_name in normalized_utterance:
            score += 8
        score += self._keyword_overlap_score(canonical_name, normalized_utterance)

        for alias in aliases:
            if alias and alias in normalized_utterance:
                score += 6
            score += self._keyword_overlap_score(alias, normalized_utterance)

        if room and self._room_token_in_utterance(device, normalized_utterance):
            score += 3

        if device_type and self._type_token_in_utterance(device_type, normalized_utterance):
            score += 2
    ...
    @staticmethod
    def _keyword_overlap_score(phrase: str, utterance: str) -> int:
        phrase_tokens = [t for t in phrase.split() if t]
        if len(phrase_tokens) < 2:
            return 0
        if all(token in utterance for token in phrase_tokens):
            return 2
        return 0
