...
class OrchestratorTests(unittest.TestCase):
    ...
    def test_expanded_shortcut_phrases(self) -> None:
        ...
        result_start = orchestrator.handle_message("start bedtime automation")
        result_run = orchestrator.handle_message("run bedtime shortcut")
        result_trigger = orchestrator.handle_message("trigger bedtime shortcut")

        self.assertIn("Shortcut handoff", result_start)
        self.assertIn("Shortcut handoff", result_run)
        self.assertIn("Shortcut handoff", result_trigger)
        self.assertEqual(0, llm.calls)

    def test_lamp_room_phrase_prefers_lamp_device(self) -> None:
        llm = FakeLLMClient(enabled=True, response="model should not be used")
        orchestrator = JarvisOrchestrator(llm_client=llm)

        result = orchestrator.handle_message("turn on the lamp in my bedroom")

        self.assertIn("bed lamp", result)
        self.assertNotIn("multiple matching devices", result)
        self.assertEqual(0, llm.calls)
